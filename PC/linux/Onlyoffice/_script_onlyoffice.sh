...
# =============================================================================================
# TITULO..: SCRIPT ONLYOFFICE
# CONTEUDO: ShellScript que instala o ONLYOFFICE no computador
...
VAR_LOG="logs/log_ONLYOFFICE_$(date +%d-%m-%Y).log"
...
figlet "ONLYOFFICE"  | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) |       ######################################      " | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) |    ############################################   " | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ##################################################" | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | #########                                #########" | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ######                                      ######" | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ###                                            ###" | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ###    DESEJA INSTALAR AS ONLYOFFICE?          ###" | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ###                                            ###" | sudo tee -a "$VAR_LOG"
...
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) |                                                   " | sudo tee -a "$VAR_LOG"
read -r "VAR_ONLYOFFICE"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) |                                                   " | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | Tu apertastes o numero: $VAR_ONLYOFFICE" | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) |                                                   " | sudo tee -a "$VAR_LOG"
echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ##################################################" | sudo tee -a "$VAR_LOG"
#####
##### Condicional de instalação ########################
#####
if [ "$VAR_ONLYOFFICE" = "3" ]; then
...
elif [ "$VAR_ONLYOFFICE" = 1 ]; then

#############################################################################################
    
    ##### Instala o ONLYOFFICE
    echo "# $(date +%d-%m-%y) ás $(date +%H:%M:%S) | ##### Instala o ONLYOFFICE"  | sudo tee -a "$VAR_LOG"

    ##### Cria a pasta que vai receber o arquivo de instalação
    sudo mkdir .onlyoffice >> "$VAR_LOG"
    
    ##### Acessa a pasta
    cd .onlyoffice/ >> "$VAR_LOG"
    
    ##### Download do arquivo
    sudo wget https://download.onlyoffice.com/install/desktop/editors/linux/old/onlyoffice-desktopeditors_amd64.deb | sudo tee -a "$VAR_LOG"
    
    ##### Concessão de permissões para o arquivo
    sudo chmod +x onlyoffice-desktopeditors_amd64.deb | sudo tee -a "$VAR_LOG"
    
    ##### Instalação
    sudo dpkg -i onlyoffice-desktopeditors_amd64.deb | sudo tee -a "$VAR_LOG"
